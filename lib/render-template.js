...
const TEMPLATE = fs.readFileSync(
  path.join(__dirname, 'Custom_Blueprint_Template.html'),
  'utf8',
);
function e(str) {
  return String(str ?? '')
  ...
  return d.captions
    .map((style, idx) => {
      const items = style.captions
        .map((cap) => `    <li>${e(cap)}</li>`)
        .join('\n');
      const breakStyle = idx === 2 ? ' style="page-break-before:always;break-before:page;padding-top:60px;"' : '';
      return `  <div class="cap-style"${breakStyle}>${e(style.style_name)}</div>
  <ul class="cap-list">
${items}
  </ul>`;
    })
...
${cards}
  ...
  <h3 style="page-break-before:always;break-before:page;padding-top:60px;">Day 1 Checklist</h3>
  <ul class="num-list">
${checklist}
  </ul>`;
}
