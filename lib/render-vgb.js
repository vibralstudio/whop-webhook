...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .cta-btn{display:inline-block;background:var(--gold);color:var(--black);font-family:'Syne',sans-serif;font-size:13px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;padding:18px 40px;text-decoration:none;margin-top:32px;}
...
function renderSection(section) {
  ...
  const ctaHtml = section.ctaButton
    ? `<div style="text-align:center;margin-top:32px;">
        <a href="${escHtml(section.ctaButton.url)}" target="_blank" class="cta-btn">${escHtml(section.ctaButton.label)}</a>
       </div>`
    : '';
